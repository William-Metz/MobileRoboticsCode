...
void loop() {
    ...
    //Do compass step to update theta
    compass.loopStep();
    forwardKinematics.setTheta(ALPHA*forwardKinematics.getPose().theta + (1 - ALPHA)*compass.getTheta());

    //Update position control 
    Pose currentPose = forwardKinematics.getPose();
    bool shouldUpdateVelocities = positionControl.loopStep(currentPose, leftVelocity, rightVelocity); //should we update velocity or no
